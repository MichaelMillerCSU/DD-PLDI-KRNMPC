...
% legend('x_1', 'reconstruncted x_1')
    ...
    figure
    plot(tspan(1:end-1), U_Test, 'LineStyle','-','LineWidth', 2.0);
...
ylabel('$||\varepsilon_k||_{\infty}$', 'Interpreter','latex')
...
hold on;
convex_p = plot(points(k,1), points(k,2), 'r-', 'LineWidth', 2);  % 凸包
axis equal;
grid on;
...
t = 0 : 0.001 : 2 * pi + 0.3;
XX = inv(R_Static) * [cos(t); sin(t)];
terminal_invariant = plot(XX(1, :), XX(2, :), 'LineWidth', 2.0, 'LineStyle','-.');
...
figure
plot(tspan(1:end-1), U, 'LineStyle','-','LineWidth', 3.0);
xlabel('$t/s$')
ylabel('$u$')
grid on
...
legend([convex_p terminal_invariant h1], {'Convex hull of the invariant sets', 'Interpolation-terminated invariant set', 'Phase trajectory with interpolated control', 'Phase trajectory with interpolation-terminated control'});




% fig = openfig('Control_DMD_Phase_Traj.fig');  % 加载已有的 figure
% hold on;                           % 允许继续画图在上面
% 
% % 画一条新的曲线，并设置 DisplayName
% 
% p_new = plot(X(1, :), X(2, :), 'LineWidth', 3.0);  % 新曲线
% name_new = 'Phase trajectory with interpolation-terminated control';
% 
% % 获取当前坐标轴
% ax = gca;
% 
...
% if ~isempty(lgd)
%     % 旧图例的句柄和标签
%     old_handles = lgd.PlotChildren(end:-1:1);         % 注意顺序反的
%     old_labels = lgd.String;
%     
%     % 追加新线和标签
%     new_handles = [old_handles; p_new];
%     new_labels = [old_labels; {name_new}];
%     
%     legend(new_handles, new_labels);  % 重新设置 legend
% else
%     % 没有 legend 就新建一个
%     legend(p_new, name_new);
% end
...
figure
plot(tspan(1:end-1),V_Set, 'LineWidth', 3.0);
xlabel('$t/s$')
ylabel('$V_3$')
grid on
